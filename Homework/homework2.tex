...
% \begin{align*}
% \du{T}_{12}^{-1} \underline{x}_1 &= \du{T}_{23} \du{T}_{34} \underline{x}_4 \\
% \du{T}_{23}^{-1} \du{T}_{12}^{-1} \underline{x}_1 &= \du{T}_{34} \underline{x}_4 \\
% \du{T}_{34}^{-1} \du{T}_{23}^{-1} \du{T}_{12}^{-1} \underline{x}_1 &= \underline{x}_4 \\
% \end{align*}

% \begin{align*}
% \du{T}_{12}^{-1} P_1 = \du{T}_{23} \du{T}_{34} P_4 \\
% \begin{bmatrix*}[c]
% x_1 ( \ \sin^2 \theta + 1) \ \cos \theta + y_1 \ \sin \theta \\
%                     -x_1 \ \sin \theta + y_1 \ \cos \theta \\
%                                              z_1 - h \\
%                                                    1 \\
% \end{bmatrix*}
...
%           1 \\
% \end{bmatrix*}\\
% \du{T}_{23}^{-1} \du{T}_{12}^{-1} P_{1}= \du{T}_{34} P_{4}\\
% \begin{bmatrix*}[c]
%                      -x_1 \ \sin \theta + y_1 \ \cos \theta \\
%                                              -z_1 + h \\
% -x_1 ( \ \sin^2 \theta + 1) \ \cos \theta - y_1 \ \sin \theta \\
...
% \begin{align*}
%  X^2 \sin^2 \theta = (Y - r)^2 \cos^2 \theta \\
%  X^2 \cos^2 \theta = (Y - r)^2 \sin^2 \theta \\
% \end{align*}
% Adding these and taking advantage of trigonometric identities
% \begin{align*}
% \left (X \right)^2 \left (\cos^2 \theta+ \sin^2 \theta \right)  &= \left( Y - r \right )^2 \left (\cos^2 \theta+ \sin^2 \theta \right) \\
% X &= Y - r
% \end{align*}
% Substituting this back in row 2 yields
% \begin{align*}
% 0 &= (Y - r) \sin \theta - Y \cos \theta + r \cos \theta \\
% 0 &= Y (\sin \theta - \cos \theta) - r (\sin \theta - \cos \theta) \\
% \alignedbox{Y}{= r}
% \end{align*}
